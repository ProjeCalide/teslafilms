...
<form method="POST" action="<?=base_url('admin');?>/<?=$page_tab;?><?if($editmi=='evet'){echo '/edit/'.$verican->yonetmen_works_id;}?>" enctype="multipart/form-data">
...
?>
		...
		<tr>
			<th valign="top">Yönetmen:</th>
			<td>
				<select name="yonetmen">
<?php
foreach($yonetmenler as $y)
{
	if($editmi=='evet')
	{
		if($y->yonetmen_id == $verican->yonetmen_id)
		{
...
?>
					<option value="<?=$y->yonetmen_id;?>" <?=$cls;?>><?=$y->isim;?></option>
<?php
}
?>
				</select>
		...
		<tr>
			<th valign="top">Film Adı:</th>
			<td><input type="text" name="film_name" value="<?if($editmi=='evet'){echo $verican->workname;}?>" class="inp-form" /></td>
			<td></td>
		</tr>
		<tr>
		<th valign="top">Resim:</th>
		<td>
<?php
	if($editmi=='evet')
	{
?>
			<img src="<?=base_url('content/images');?>/<?=$verican->image;?>">
<?php
	}
?>
		...
		<td>
			...
			<div class="bubble-inner">Resim 4 Frame, Frame boyutları 180x100, Toplam Resim Boyutu 180x400, Format JPG, Olmalıdır!</div>
